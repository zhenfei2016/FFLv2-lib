...
extern "C" {
	 ...
	 *
	 */
	extern DECLSPEC FFL_Threadpool FFL_CALL FFL_CreateThreadpool(int threadnum);

	/*
	 * 运行一个job，如果当前没有可用的线程则等待
	 * 返回是否成功呢  FFL_ERROR_SUCCESS：成功
	 */
	extern DECLSPEC int FFL_CALL FFL_RunThreadpoolJob(FFL_Threadpool pool, FFL_ThreadpoolJob* job);

	/*
	 * 结束线程池
	 * 等待所有的线程结束后才返回
	 */
	extern DECLSPEC void FFL_CALL  FFL_DetachThreadpool(FFL_Threadpool  pool);
