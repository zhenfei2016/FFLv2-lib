...
{
    FFL_sem *sem = (FFL_sem *) FFL_mallocz(sizeof(FFL_sem));
    if (sem) {
        if (sem_init(&sem->sem, 0, initial_value) < 0) {
            FFL_set_error("sem_init() failed");
            FFL_free(sem);
            sem = NULL;
        }
    } else {
...
int FFL_SemWait(FFL_sem * sem)
{
    ...
    if (retval < 0) {
        retval = FFL_set_error("sem_wait() failed");
    }
    return retval;
}
    ...
    */
    ...
    if (retval < 0) {
        if (errno == ETIMEDOUT) {
            ...
            FFL_set_error("sem_timedwait returned an error: %s", strerror(errno));
        }
    }
#else
