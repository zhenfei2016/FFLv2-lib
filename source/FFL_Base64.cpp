...
*
*  FFL_Base64.cpp
*  Created by zhufeifei(34008081@qq.com) on 2019/01/17
*  https://github.com/zhenfei2016/FFLv2-lib.git
*
*  base64封装websockettpp中代码
*
*/
...
inline std::string base64_decode(std::string const & input) {
  ...
  //  编码，解码
  //
extern "C" status_t FFL_Base64Encode(const uint8_t * input, size_t len, uint8_t* output, size_t outBufSize) {
	if (input == NULL || output == NULL || (len * 6) > (outBufSize * 8)) {
		return FFL_FAILED;
	}

	std::string enc=websocketpp::base64_encode(input,len);
	memcpy(output, enc.c_str(), enc.size());
	if (outBufSize > enc.size()) {
		output[enc.size()] = 0;
	}
	return FFL_OK;
}
extern "C" status_t FFL_Base64Decode(const uint8_t * input, size_t len, uint8_t* output, size_t outBufSize) {
	...
	inputBase64.append((const char*)input, len);

	std::string dec = websocketpp::base64_decode(inputBase64);
	memcpy(output, dec.c_str(), dec.size());
	if (outBufSize > dec.size()) {
		output[dec.size()] = 0;
	}
	return FFL_OK;
}
